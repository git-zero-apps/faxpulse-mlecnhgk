  ...
  if (error) {
    return (
      <div className="rounded-lg bg-red-50 border border-red-200 p-4">
        <p className="text-sm text-red-700">Error loading contacts: {error.message}</p>
      </div>
    );
  }
    ...
    <div>
      ...
      {items && items.length > 0 ? (
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Fax Number</th>
            ...
            <tbody className="divide-y divide-gray-200">
              {items.map((item: Record<string, unknown>) => (
                <tr key={item.id as string} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-sm text-gray-900 whitespace-nowrap">{String(item.fax_number ?? "—")}</td>
                  ...
                  <td className="px-6 py-4 text-right text-sm whitespace-nowrap">
                    <Link href={`/dashboard/contacts/${item.id}/edit`} className="text-brand-600 hover:text-brand-800 font-medium mr-4">
                      Edit
                    </Link>
                    <DeleteButton id={item.id as string} table="contacts" />
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        <div className="card text-center py-12">
          ...
          <h3 className="mt-4 text-sm font-semibold text-gray-900">No contacts</h3>
          <p className="mt-1 text-sm text-gray-500">Get started by adding your first contact.</p>
          <Link href="/dashboard/contacts/new" className="btn-primary mt-6 inline-flex">
            Add Contact
          </Link>
        </div>
      )}
