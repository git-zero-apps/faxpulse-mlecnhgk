...
export default function NewContactPage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const { error: insertError } = await supabase.from("contacts").insert(record);
  ...
  return (
    <div className="max-w-2xl">
      <div className="mb-8">
        <Link href="/dashboard/contacts" className="text-sm text-gray-500 hover:text-gray-700 mb-2 inline-flex items-center gap-1">
          <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" d="M15.75 19.5L8.25 12l7.5-7.5" />
          </svg>
          Back to Contacts
        </Link>
        <h1 className="text-2xl font-bold text-gray-900 mt-2">Add Contact</h1>
      </div>
      ...
      <form onSubmit={handleSubmit} className="card space-y-6">
        ...
        <div>
          <label htmlFor="fax_number" className="label">Fax Number</label>
          <input id="fax_number" name="fax_number" type="text" className="input" placeholder="Enter fax number" required />
        </div>
        <div>
        ...
        <div className="flex items-center gap-3 pt-4 border-t">
          ...
          <Link href="/dashboard/contacts" className="btn-secondary">Cancel</Link>
        </div>
      </form>
    </div>
  );
